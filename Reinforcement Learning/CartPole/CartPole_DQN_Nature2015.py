...
_L1=tf.nn.relu(tf.matmul(x,W1))
L1=tf.nn.dropout(_L1,dropout)
_L2=tf.nn.relu(tf.matmul(L1,W2))
...
with tf.Session() as sess:
    ...
    # 에피소드 시작
    while np.mean(recent_rlist) < 1000 :
        episode += 1
        ...
        if episode % 10 == 1 and len(REPLAY_MEMORY) > 10:
            ...
            for i in range(MINIBATCH):
                ...
                for sample in ran.sample(REPLAY_MEMORY, REPLAY):
                    ...
                    sess.run(train, feed_dict={x: s_t_r, y: Q, dropout:0.7})
